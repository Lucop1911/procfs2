...
#[proc_macro_derive(ProcKeyValue, attributes(proc_key))]
pub fn proc_key_value_derive(input: TokenStream) -> TokenStream {
    ...
    let fields = match &input.data {
        Data::Struct(s) => match &s.fields {
            ...
            _ => {
                return syn::Error::new_spanned(
                    &input.ident,
                    "ProcKeyValue only supports structs with named fields",
                )
                .to_compile_error()
                .into();
            }
        ...
        _ => {
            return syn::Error::new_spanned(&input.ident, "ProcKeyValue only supports structs")
                .to_compile_error()
                .into();
        }
    };
    ...
    for field in fields {
        ...
        field_parsers.push(quote! {
            #proc_key => {
                __instance.#field_name = #parse_expr;
            }
        });
    ...
    let expanded = quote! {
        impl #impl_gen procfs2::util::parse::ParseFromBytes for #name #ty_gen #where_gen {
            fn parse_from_bytes(bytes: &[u8]) -> procfs2::error::Result<Self> {
        ...
        if let Some(segment) = type_path.path.segments.last() {
            match segment.ident.to_string().as_str() {
                "Kibibytes" => {
                    quote! {
                        Kibibytes(
                            procfs2::util::parse::parse_dec_u64(
                                procfs2::util::parse::first_token(trim_start(value))
                            ).unwrap_or(0)
                        )
                    }
                }
                ...
                "u32" => {
                    quote! {
                        procfs2::util::parse::parse_dec_u32(
                            procfs2::util::parse::first_token(trim_start(value))
                        ).unwrap_or(0)
                    }
                }
                ...
                "i32" => {
                    quote! {
                        procfs2::util::parse::parse_dec_i32(
                            procfs2::util::parse::first_token(trim_start(value))
                        ).unwrap_or(0)
                    }
                }
                "i64" => {
                    quote! {
                        procfs2::util::parse::parse_dec_i64(
                            procfs2::util::parse::first_token(trim_start(value))
                        ).unwrap_or(0)
                    }
                }
                "f32" => {
                    quote! {
                        procfs2::util::parse::parse_dec_f32(
                            procfs2::util::parse::first_token(trim_start(value))
                        ).unwrap_or(0.0)
                    }
                }
                "f64" => {
                    quote! {
                        procfs2::util::parse::parse_dec_f64(
                            procfs2::util::parse::first_token(trim_start(value))
                        ).unwrap_or(0.0)
                    }
                }
